...
contains
   ...
   subroutine init_anelastic_Q8_properties(M, G, infile)
      ...
      !   c     : Q_S = c * V_S  (default 1.0)
      !   cp    : Q_P = cp * V_P (optional; if not set, i.e. cp <= 0, falls back to
      !           the legacy hard-coded ratio Q_P = 2*Q_S, preserving prior behavior)
      !   fref  : reference frequency (Hz) for unrelaxed modulus correction (default 1.0)
      ...
      real(kind = wp) :: c, cp, fref
      real(kind = wp) :: taumin, taumax, wref
      real(kind = wp) :: val_S, val_P, denom_S, denom_P, vs, vp, mu_unrelax_S, mu_unrelax_P
      integer :: stat, i, l, j, k, N
      real(kind = wp), parameter :: pi = 3.141592653589793_wp

      namelist /anelastic_Q8_list/ c, cp, fref

      ! Defaults
      c    = 1.0_wp
      cp   = -1.0_wp   ! sentinel: cp<=0 means "not set" -> legacy Q_P = 2*Q_S
      fref = 1.0_wp
      ...
      do i = G%C%mq, G%C%pq
         do j = G%C%mr, G%C%pr
            do k = G%C%ms, G%C%ps
               ...
               if (cp > 0.0_wp) then
                  ! Q_P set independently via cp: Q_P = cp * V_P
                  M%Qp_inv_Q8(i,j,k) = 1.0_wp / (cp * sqrt((M%M(i,j,k,1) + 2.0_wp*M%M(i,j,k,2)) / M%M(i,j,k,3)))
               else
                  ! Legacy behavior: Q_P = 2*Q_S
