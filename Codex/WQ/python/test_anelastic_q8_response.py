...
MATERIAL = ROOT / "src" / "material.f90"
NMECH = 8
TARGET_Q = 50.0
FMIN = 0.08
FMAX = 15.0
...
def read_weights() -> list[float]:
    ...
    weights: list[float] = []
    for index in range(1, NMECH + 1):
        match = re.search(
            rf"M%weight_Q8\({index}\)\s*=\s*"
            rf"([-+0-9.eEdD]+)_wp",
            text,
        )
        if match is None:
            raise AssertionError(f"weight_Q8({index}) not found")
        weights.append(float(match.group(1).replace("d", "e").replace("D", "E")))
    return weights
