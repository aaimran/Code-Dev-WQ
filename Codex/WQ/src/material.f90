...
contains
   ...
   subroutine init_anelastic_Q8_properties(M, G, infile)
      ...
      !   Qs0, Qp0 : constant S- and P-wave Q. If both are positive, these
      !              values are used directly at every material point.
      !   c         : backward-compatible mode used only when Qs0 and Qp0
      !              are omitted; Q_S = c*V_S and Q_P = 2*Q_S.
      !   fref      : reference frequency (Hz) for unrelaxed-modulus correction.

      use mpi3dcomm, only : allocate_array_body
      use mpi3dbasic, only : error, warning, is_master
      ...
      real(kind = wp) :: c, fref, Qs0, Qp0
      real(kind = wp) :: taumin, taumax, wref
      real(kind = wp) :: val_S, val_P, denom_S, denom_P, vs, vp, mu_unrelax_S, mu_unrelax_P
      integer :: stat, i, l, j, k, N
      logical :: explicit_q_requested
      real(kind = wp) :: max_qs_error, max_qp_error
      real(kind = wp), parameter :: pi = 3.141592653589793_wp

      namelist /anelastic_Q8_list/ c, Qs0, Qp0, fref
      ...
      if (stat > 0) call error('error reading namelist anelastic_Q8_list', &
                               'init_anelastic_Q8_properties')

      explicit_q_requested = (Qs0 /= -1.0_wp .or. Qp0 /= -1.0_wp)
      if (explicit_q_requested .and. (Qs0 <= 0.0_wp .or. Qp0 <= 0.0_wp)) then
         call error('Qs0 and Qp0 must both be positive when either is specified', &
                    'init_anelastic_Q8_properties')
      end if
      if (.not. explicit_q_requested .and. c <= 0.0_wp) then
         call error('legacy parameter c must be positive', &
                    'init_anelastic_Q8_properties')
      end if
      if (fref <= 0.0_wp) then
         call error('fref must be positive', 'init_anelastic_Q8_properties')
      end if

      M%explicit_Q8 = explicit_q_requested
      M%Qs0_Q8 = Qs0
      M%Qp0_Q8 = Qp0
      M%fref_Q8 = fref
      ...
      ! Set inverse Q. Explicit Q is the preferred mode and permits Q_P = Q_S.
      ! The velocity-scaled c mode is retained for existing input files.
      do i = G%C%mq, G%C%pq
         do j = G%C%mr, G%C%pr
            do k = G%C%ms, G%C%ps
               ...
               if (explicit_q_requested) then
                  M%Qs_inv_Q8(i,j,k) = 1.0_wp / Qs0
                  M%Qp_inv_Q8(i,j,k) = 1.0_wp / Qp0
               else
                  M%Qs_inv_Q8(i,j,k) = 1.0_wp / &
                       (c * sqrt(M%M(i,j,k,2)/M%M(i,j,k,3)))
      ...
      ! Nonnegative constant-Q weights for the transfer function implemented
      ! in apply_anelastic_Q8_point. They are normalized and multiplied by
      ! inverse Q exactly once in the RHS. The fit is optimized at Q=50 over
      ! [0.08, 15] Hz; its maximum realized-Q error there is below 5%.
      M%weight_Q8(1) = 1.50589707_wp
      M%weight_Q8(2) = 0.0_wp
      M%weight_Q8(3) = 0.52793567_wp
      M%weight_Q8(4) = 0.53065494_wp
      M%weight_Q8(5) = 0.32862132_wp
      M%weight_Q8(6) = 0.64375916_wp
      M%weight_Q8(7) = 0.0_wp
      M%weight_Q8(8) = 1.32751442_wp
      ...
      if (is_master(G%C%comm)) then
         write(*,'(A)') 'anelastic-Q8 parameters:'
         if (explicit_q_requested) then
            write(*,'(A,ES12.4,A,ES12.4)') '  Qs0 = ', Qs0, ', Qp0 = ', Qp0
         else
            write(*,'(A,ES12.4,A)') '  legacy c = ', c, &
                 ' (Qs=c*Vs, Qp=2*Qs)'
         end if
         write(*,'(A,ES12.4)') '  fref (Hz) = ', fref
         write(*,'(A,8(ES12.4,1X))') '  tau (s) = ', M%tau_Q8
         write(*,'(A,8(ES12.4,1X))') '  normalized weights = ', M%weight_Q8
         if (explicit_q_requested) then
            call q8_max_relative_error(Qs0, M%tau_Q8, M%weight_Q8, &
                                       0.08_wp, 15.0_wp, max_qs_error)
            call q8_max_relative_error(Qp0, M%tau_Q8, M%weight_Q8, &
            ...
            if (max(max_qs_error, max_qp_error) > 0.05_wp) then
               call warning('anelastic-Q8 realized-Q error exceeds 5 percent; '// &
                            'the fixed weights are optimized for Q=50', &
                            'init_anelastic_Q8_properties')
            end if
         end if
      end if
   ...
   subroutine q8_max_relative_error(target_Q, tau, weight, fmin, fmax, max_error)

      implicit none

      real(kind = wp), intent(in) :: target_Q, tau(8), weight(8), fmin, fmax
      real(kind = wp), intent(out) :: max_error

      integer, parameter :: nfreq = 256
      integer :: i, l
      real(kind = wp) :: f, omega, x, modulus_real, modulus_imag, q_model
      real(kind = wp), parameter :: pi = 3.141592653589793_wp
      ...
      do i = 1, nfreq
         ...
         if (modulus_imag <= 0.0_wp) then
            max_error = huge(1.0_wp)
            return
         end if
         q_model = abs(modulus_real/modulus_imag)
         max_error = max(max_error, abs(q_model/target_Q - 1.0_wp))
      end do
